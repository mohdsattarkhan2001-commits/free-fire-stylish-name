...
function generate() {
  ...
  const styled = getStyledNames(name);
  const presets = name ? buildPresets(name) : [];

  renderList(resultsList, styled.length ? styled : ["Type a name above to see styled results"]);
  renderList(presetList, presets.length ? presets : ["Preset formats will appear here"]);
}
